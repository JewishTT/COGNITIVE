...
- Added a **TDA-анализ** tab to the platform's ОСИНТ section: a new Topological
  Data Analysis layer over the native investigation graph. It runs connected
  components (H0), cycle rank + fundamental cycles (H1), Tarjan bridges and
  cut vertices, Brandes betweenness + k-core centrality, and spatial
  single-linkage clustering with a 0-dimensional persistence barcode. Selecting
  any component/cluster/cycle/cut vertex in the panel highlights the matching
  nodes on the graph canvas. The existing «Граф связей» iframe tab is unchanged
  (hybrid: both tabs coexist in the same section). Implementation:
  `platform/src/osint/tda.ts` (engine), `platform/src/osint/TdaPanel.vue`
  (the layer panel), and wiring in `GraphCanvas.vue`/`OsintView.vue`.
- Added a **simplicial-complex view** and **demo seed data** to the TDA layer.
  The engine now reports 2-simplices (triangles of the clique/flag complex),
  and the new «Комплекс» tab renders the complex as an SVG stage (vertices as
  0-simplices, edges as 1-simplices, filled triangles as 2-simplices). A
  «Демо: симплициальный комплекс» button seeds a backend-free test graph
  (two glued triangles, a 4-cycle hole with H1, a pendant edge, an isolated
  vertex) so the topological structure can be inspected without a live
  Flowsint API. Implementation: `platform/src/osint/tda.ts` (triangle
  detection), `platform/src/osint/demo.ts`, `platform/src/osint/TdaPanel.vue`,
  `platform/src/osint/GraphCanvas.vue`.
  `platform/src/osint/TdaPanel.vue` and wiring in
  `platform/src/views/OsintView.vue`.
- **Reworked the TDA tab into a full-page 3D view.** The simplicial complex is
  now rendered only in 3D: the Cesium `Tda3D` stage is the main page content
  (grid: investigations sidebar | large 3D stage | compact metrics panel), the
  2D VueFlow canvas no longer appears in the TDA tab, and the 2D SVG complex
  view was removed. `OsintView` loads the sketch graph directly from the
  Flowsint API, and the demo seed lives in the stage's empty state.
- **Rebuilt the 3D scene visuals.** Removed the accidental "constellation in
  space" look by disabling Cesium's default starfield, atmosphere, sun and
  moon. The complex now sits above a hologram grid floor
  (`GridMaterialProperty`), edges render as glowing struts
  (`PolylineGlowMaterialProperty`), vertices are solid shaded spheres, and
  2-simplices are tinted translucent facets with lit edges. Camera frames the
  complex from a 3/4 tabletop angle; panel selections highlight the matching
