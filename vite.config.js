...
/**
 * Корень сервера = оболочка платформы, а автономный глобус живёт на /globe.
 *
 * В dev и preview `/` отдаёт platform.html (сайдбар с модулями Глобус / ОСИНТ /
 * Фабрика), а сам глобус по-прежнему доступен по отдельному пути /globe —
 * его и встраивает iframe раздела «Глобус». Без переписывания корень отдавал
 * бы автономный index.html-глобус, и оболочка была бы только на platform.html.
 */
function platformRootShell() {
  const install = (server) => {
    server.middlewares.use((req, res, next) => {
      if (!req.url) return next();
      const pathname = new URL(req.url, 'http://gev.local').pathname;
      if (pathname === '/' || pathname === '/platform.html') {
        req.url = '/platform.html';
      } else if (pathname === '/globe' || pathname === '/globe.html') {
        req.url = '/index.html';
      }
      next();
    });
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    resolve: {
      alias: {
        // Алиас SPA-платформы (Vue) — разделы, встроенные в one page.
    ...
    server: {
      ...
      // Встроенные разделы платформы: ОСИНТ (Flowsint) и Фабрика (Postiz) —
      // отдельные процессы за отдельными портами, но открываются прямо на
      // странице, без внешних ссылок. Глобус обслуживает этот же сервер
      // (страница "/globe"), поэтому его iframe в разделе «Глобус» — same-origin.
      proxy: {
        '/factory': {
          target: 'http://localhost:4007',
          changeOrigin: true,
          rewrite: (p) => p.replace(/^\/factory/, ''),
          // Postiz ставит auth-куку с Domain=localhost. Если платформа открыта
          // на другом имени хоста (127.0.0.1, LAN-IP), браузер такую куку
          // отвергает, и встроенная Фабрика падает в форму входа. Нормируем
          // cookie до host-only: она валидна для всего хоста на любом порту —
          // в том числе для iframe Фабрики на :4007.
          configure: (proxy) => {
            proxy.on('proxyRes', (proxyRes) => {
              const setCookie = proxyRes.headers['set-cookie'];
              if (!setCookie) return;
              const strip = (c) => c.replace(/;\s*Domain=[^;]*/i, '');
              proxyRes.headers['set-cookie'] = Array.isArray(setCookie) ? setCookie.map(strip) : strip(setCookie);
            });
          },
        },
      },
    ...
    define: {
      ...
      // Встроенная Фабрика (Postiz): авто-вход администратора. Учётка создаётся
      // прозрачно при первом запуске (регистрация разрешена ровно пока в БД 0
      // организаций). Переопределяется через .env / POSTIZ_ADMIN_EMAIL.
      'import.meta.env.POSTIZ_ADMIN_EMAIL': JSON.stringify(env.POSTIZ_ADMIN_EMAIL || 'admin@gev.local'),
      'import.meta.env.POSTIZ_ADMIN_PASSWORD': JSON.stringify(env.POSTIZ_ADMIN_PASSWORD || 'PostizAdmin123!'),
      // Встроенный ОСИНТ (Flowsint): авто-вход под существующим аккаунтом
      // платформы. Пароль аккаунта сброшен на этот seed (см. flowsint DB).
      // Переопределяется через .env / FLOWSINT_ADMIN_EMAIL.
      'import.meta.env.FLOWSINT_ADMIN_EMAIL': JSON.stringify(env.FLOWSINT_ADMIN_EMAIL || 'delovadrin@gmail.com'),
      'import.meta.env.FLOWSINT_ADMIN_PASSWORD': JSON.stringify(env.FLOWSINT_ADMIN_PASSWORD || 'PostizAdmin123!'),
      // Встроенный Глобус: в production оболочка и глобус — один сервер, поэтому
      // iframe грузит глобус по same-origin пути /globe. В dev это переопределяется
      // рантайм-фолбэком на http://<host>:4173/globe (см. GlobeView.vue).
      'import.meta.env.GLOBE_EMBED_URL': JSON.stringify('/globe'),
    },
    build: {
      // The Cesium engine bundle is inherently large; raise the warning ceiling
      // so the build log isn't dominated by an expected chunk-size notice.
      chunkSizeWarningLimit: 1500,
      // Один билд: страница платформы "/" (оболочка, где Глобус / ОСИНТ /
      // Фабрика открываются прямо «в рамке» страницы) + автономная страница
      // глобуса index.html, доступная для встраивания по пути /globe.
      rollupOptions: {
        input: {
          globe: path.resolve(__dirname, 'index.html'),
