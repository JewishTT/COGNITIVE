...
class ModuleLoader:
    ...
    def _write_secret_text(path, text):
        """_gev_windows_fix: Windows-safe secrets writer.
        POSIX chmod/mode checks don't apply on NTFS; mkstemp + os.replace is
        already owner-only per Windows ACLs. Keeps the atomic-replace contract."""
        path = Path(path)
