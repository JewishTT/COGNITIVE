...
# _gev_lazy_ansible: ansible_runner imported lazily in ansible_run()
...
class DepsInstaller:
    ...
    def ansible_run(self, tasks=None, module=None, args=None, ansible_args=None):
        ...
        try:
            ...
            raise RuntimeError(
                "Ansible-based dependency installation is not available "
                f"on this platform ({_e}). System-level deps (apt/etc.) are "
                "Linux-only; pip deps are unaffected."
            ) from _e
        res = run(
            playbook=playbook,
    ...
    async def install_core_deps(self):
        if os.name == "nt":  # _gev_skip_core_deps_win: ansible system-deps are Linux-only
